...
const CompositionService = () => {
  const create = async (parentId, payload) => {
    if (payload.material_id === parentId) {
      throw badRequestError("Cannot set the composition of a material to itself");
    }
    ...
    const existingComposition = await Composition.get(parentId, payload.material_id);
    ...
    const parentCompositions = await Composition.getParentCompositions(parentId);
    const parentIds = parentCompositions.map(composition => composition.parent_id);
    const updatedWeapons = await Weapon.recalculatePowerLevels([newComposition.parent_id, ...parentIds]);
  ...
  const update = async (parentId, materialId, payload) => {
    ...
    const updatedComposition = await Composition.update(parentId, materialId, payload);
    ...
    const parentCompositions = await Composition.getParentCompositions(updatedComposition.parent_id);
    const parentIds = parentCompositions.map(composition => composition.parent_id);
    const updatedWeapons = await Weapon.recalculatePowerLevels([updatedComposition.parent_id, ...parentIds]);
  ...
  const remove = async (parentId, materialId) => {
    ...
    const parentCompositions = await Composition.getParentCompositions(deletedComposition.parent_id);

    const parentIds = parentCompositions.map(composition => composition.parent_id);
    const updatedWeapons = await Weapon.recalculatePowerLevels([deletedComposition.parent_id, ...parentIds]);
